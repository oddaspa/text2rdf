class TripleExtractor:
    ...
    def isValidTokens(self, tokens):
        are_any_tokens_in_stop_words = any(
            item in self.getNER(tokens) for item in self.STOP_WORDS
        )
    ...
    def get_pref_entity(self, triple):
        ...
        if not obj_ners.count(obj_ners[0]) == len(obj_ners):
            new_obj = np.array(triple.object.split(" "))[
                self.get_pref_ner_index(np.array(obj_ners))
            ]
        ...
        if not sub_ners.count(sub_ners[0]) == len(sub_ners):
            new_sub = np.array(triple.subject.split(" "))[
                self.get_pref_ner_index(np.array(sub_ners))
            ]
